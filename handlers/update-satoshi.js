...
const debug = require("debug")("handler_update_satoshi");

module.exports = (socket) => {
	return (data, ack) => {
		debug("received update satoshi request");
		if(!data.streamId) {
		...
		.then((apiServer) => {
			...
			return apiServer.socket.sign("update satoshi", data, true);
		})
		.then((response) => {
			debug("got update satoshi response");
			return socket.signAck(ack, response);
		})
		.catch((err) => {
			if(err instanceof Error) {
				console.error("[UpdateSatoshi]", err);
